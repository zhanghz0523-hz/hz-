...
function heroImageProps(p){
 ...
 const sizes=`max(100vw, ${p.ratio*100}svh, ${p.ratio*780}px)`;
 return {src:imageUrl(src,2560,90),srcSet:[1200,1600,2000,2560,3000,3500,4000].map(w=>`${imageUrl(src,w,90)} ${w}w`).join(', '),sizes};
}
...
const CDN_WIDTHS=[480,768,1200,1600,2000];
const isLocalPreview=typeof window!=='undefined' && /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname);
function imageUrl(src,width,quality=78){
 if(!import.meta.env.PROD||isLocalPreview||!src.startsWith('/assets/'))return src;
 const params=new URLSearchParams({url:src,w:String(width),q:String(quality)});
 return `/.netlify/images?${params.toString()}`;
}
function imageProps(src,{width=1600,sizes='100vw',quality=78}={}){
 if(!import.meta.env.PROD||isLocalPreview)return {src};
 return {src:imageUrl(src,width,quality),srcSet:CDN_WIDTHS.map(w=>`${imageUrl(src,w,quality)} ${w}w`).join(', '),sizes};
}
