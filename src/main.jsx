...
function heroImageProps(p){
 return {src:`/assets/hero-${p.id}-1600.webp`,srcSet:[960,1600,2400,4000].map(w=>`/assets/hero-${p.id}-${w}.webp ${Math.min(w,{space:3840,wangshan:3500,yosemite:3001,lighting:4000}[p.id])}w`).join(', '),sizes:`max(100vw, ${p.ratio*100}svh)`};
}
...
function Home({slide,onNext}){
 ...
 useEffect(()=>{if(loaded.includes(slide))setVisible(slide)},[slide,loaded]);
 const imageLoaded=index=>{setLoaded(current=>current.includes(index)?current:[...current,index]);setRequested(current=>Math.max(current,Math.min(index+2,heroProjects.length)));};
 const handleKeyDown=(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();onNext()}}
 return <main className="home-page"><section className="hero hero-typographic" id="home" onClick={onNext} onKeyDown={handleKeyDown} role="button" tabIndex={0} aria-label="切换首页封面"><div className="hero-slideshow" aria-hidden="true">{heroProjects.slice(0,requested).map((item,index)=><img key={item.id} className={`hero-slide ${index===visible?'active':''}`} {...heroImageProps(item)} alt="" loading="eager" fetchPriority={index===0?'high':'low'} onLoad={()=>imageLoaded(index)} onError={()=>setRequested(current=>Math.min(current+1,heroProjects.length))} decoding="async"/>)}</div><div className="hero-shade"/><div className="hero-art-copy wrap"><h1 aria-label="ZHANG HONGZHI"><span>ZHANG</span><span>HONGZHI</span></h1><p className="hero-art-meta">INTERIOR DESIGNER<br/>SPACE · LIGHT · MATERIAL<br/>SELECTED WORKS<br/>2022 — 2026</p></div><div className="hero-art-foot wrap"><span>FEATURED / {heroProjects[visible].name} <i/></span><span>{String(visible+1).padStart(2,'0')} / {String(heroProjects.length).padStart(2,'0')}</span></div></section></main>
}
